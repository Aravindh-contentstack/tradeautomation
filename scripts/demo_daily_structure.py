"""Runs the three Daily tiers on synthetic Daily candles and prints them.

Why this does not reuse demo_swing_structure.py's generator
-----------------------------------------------------------
build_synthetic_data() advances one CALENDAR DAY per candle too, but its
segments are engineered specifically to trip the OLD lookback tier's rules
...
Daily's three tiers no longer use any of those rules (no lookback window,
no timeout, no hold) as of the July 29 2026 port to the fractal-family
mechanism, so that fixture would be testing machinery that is not there
any more.
...
Why this reuses the 4H fixture's exact price targets and leg constants
------------------------------------------------------------------------
The n=2/8/20 values are unchanged from h4_structure.py's own (see
daily_structure.py's docstring: n=20 is in fact the value ORIGINALLY
observed on Daily, n=8 is carried over from 4H without Daily-specific
justification). Every leg/pull constant below (MINOR_PULL_STEPS, advance
and retrace candle counts, the price targets) exists purely to separate
the three n values by BAR COUNT, not by wall-clock time or price level: a
pull has to exceed n=2 candles' worth of progress and stay under n=8's,
regardless of whether one candle spans one day or four hours. Since n is
unchanged, none of that arithmetic needs to change either, so this
fixture is deliberately identical to demo_h4_structure.py's other than the
candle spacing (one candle per calendar day, not six per day) and the
daily_ import.

Run from the repo root:  python scripts/demo_daily_structure.py
"""
...
# One Daily candle is one calendar day. No weekend candles, which is what
# real forex Daily data looks like and what the FXR scripts' bar spacing
# inference expects to see (their rule takes the MINIMUM gap between
# candles precisely so a weekend gap cannot fool it).
...
# absolute price move. Unchanged from demo_h4_structure.py: this is
# bar-count-relative, not timeframe-relative, and n is unchanged.
MINOR_PULL_STEPS = 2.6


def _advance(moment):
    """Next Daily candle timestamp, skipping Saturday and Sunday."""
...
def build_synthetic_daily_data():
    """Builds Daily candles with deliberately separated swing scales.
    ...
    scales are present rather than only the fastest one. Identical target
    prices and leg constants to demo_h4_structure.py's fixture, since n is
    unchanged and every constant here is bar-count-relative, not
    timeframe-relative (see this module's docstring).
    """
    rows = []
    ...
    # shape compute_daily_structures expects, and to prove a restart on one
    # tier is expressible without touching the others.
    manual_restarts = {
        tier: pd.Series([False] * len(df)) for tier in DAILY_TIER_PERIODS
    }
...
def main():
    ...
    for tier in tiers:
        print(
            "  %-14s n=%-3d pivot updates: %3d   structure flips: %d"
            % (
                tier,
                DAILY_TIER_PERIODS[tier],
    ...
    for _, row in result.iterrows():
        ...
        print(
            ...
            % (
                ...
                _state(row, "daily_fractal"),
                _state(row, "daily_internal"),
                _state(row, "daily_swing"),
            )
        )
    print()
