"""Runs the three H1 tiers on synthetic H1 candles and prints them.
...
The H1 tiers use none of those rules (no lookback window, no timeout, no
...
------------------------------------------------------------------------
The n=2/8/20 values are unchanged from h4_structure.py (see
h1_structure.py's docstring: they are a placeholder, not yet recalibrated
for H1). Every leg/pull constant below (MINOR_PULL_STEPS, advance and
retrace candle counts, the price targets) exists purely to separate the
three n values by BAR COUNT, not by wall-clock time or price level: a
pull has to exceed n=2 candles' worth of progress and stay under n=8's,
regardless of whether one candle spans one hour or four. Since n is
unchanged, none of that arithmetic needs to change either, so this
fixture is deliberately identical to demo_h4_structure.py's other than
the candle spacing and the h1_ import. That is a decision, not an
oversight: a genuinely H1-specific fixture would only be needed once the
regime testing calls for different n values.

Run from the repo root:  python scripts/demo_h1_structure.py
"""
...
# One H1 candle is one hour. About twenty-four per weekday, none at the
# weekend, which is what real forex H1 data looks like and what the FXR
# scripts' bar spacing inference expects to see (their rule takes the
# MINIMUM gap between candles precisely so a weekend gap cannot fool it).
CANDLE_HOURS = 1
CANDLES_PER_DAY = 24
...
def _advance(moment):
    """Next H1 candle timestamp, skipping Saturday and Sunday."""
...
def build_synthetic_h1_data():
    """Builds H1 candles with deliberately separated swing scales.
    ...
    # shape compute_h1_structures expects, and to prove a restart on one
    # tier is expressible without touching the others.
    manual_restarts = {
        tier: pd.Series([False] * len(df)) for tier in H1_TIER_PERIODS
    }
    ...
    for _, row in result.iterrows():
        ...
        print(
            ...
            % (
                ...
                _state(row, "h1_fractal"),
                _state(row, "h1_internal"),
                _state(row, "h1_swing"),
            )
        )
    print()
