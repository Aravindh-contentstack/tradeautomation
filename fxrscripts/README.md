...
Six more scripts provide equilibrium visualizations, one swing and one internal per timeframe:
- [`daily_swing_premium_discount.py`](daily_swing_premium_discount.py) (orange)
- [`daily_internal_premium_discount.py`](daily_internal_premium_discount.py) (cyan)
- [`h4_swing_premium_discount.py`](h4_swing_premium_discount.py) (coral)
- [`h4_internal_premium_discount.py`](h4_internal_premium_discount.py) (azure)
- [`h1_swing_premium_discount.py`](h1_swing_premium_discount.py) (gold)
- [`h1_internal_premium_discount.py`](h1_internal_premium_discount.py) (turquoise)

Each is a full duplicate of the matching structure script's fractal-detection engine (e.g. `h4_swing_structure.py`'s n=20, `h4_internal_structure.py`'s n=8) with a drawing block appended. They draw a single thin ("EQ"-labeled) `horizontalLine` at that tier's current equilibrium price, `(effectiveHigh + effectiveLow) / 2` (see below), nothing else.

An earlier version of the 4H scripts also drew a shaded, "PREMIUM"/"DISCOUNT"-labeled `rectangle` spanning the range's two corners in time. That box's left corner sat at whichever pivot's own (possibly old) timestamp, so as the chart accumulated history the box visually grew to cover every prior swing leg rather than just the current one, reading as premium/discount for every past structure at once instead of just the latest, which is not what was wanted. Removed entirely rather than fixed, since a `horizontalLine` has no time corners to get wrong in the first place, it is simply the current equilibrium price, full chart width, always current, and it is deleted and redrawn every closed candle (`deleteDrawingByCondition` then redraw) the same way the box was, since the price itself can still change candle to candle. Each of the six scripts reserves its own `EQ_LINE_COLOR` (all six pairwise distinct, both linewidth 1), matched via `opts['linecolor']`, so no script's cleanup can touch another script's line, on any timeframe, or any trendLine drawn by the structure scripts. Earlier, the swing-tier scripts all shared orange and the internal-tier scripts all shared cyan across timeframes, which meant a script running on the wrong timeframe would delete a same-tier sibling's correctly-drawn line on every mismatch tick. Fixed by giving each of the six its own color.

There is no existing TradingView reference indicator for this the way there is for swing/internal/fractal structure, so these six scripts are themselves the visual verification method: load one in fxreplay, and check the line's price against the tier's own swing high/low and current direction by eye.

**All six scripts recalibrate as the trend runs.** Each tier's raw `swing_high`/`swing_low` are Williams Fractal pivots that only update once fully confirmed (`periods` candles beyond it with no reversal), and `h4_swing_structure.py`'s own docstring already documents that this makes a tier "go quiet in a strong one-directional trend": once a side breaks and price keeps running, that pivot freezes at the already-broken level (the same applies to every internal tier's pivots, just not documented there in as many words). Using either raw column directly would freeze the equilibrium right along with it instead of recalibrating as the trend extends. The fix (`swing_structure/current_range.py`'s `compute_current_range` in Python, ported into each script as `{prefix}RunningMaxHigh`/`{prefix}RunningMinLow`) extends whichever side is stale with the running high (or low) actually made since that side's pivot last changed, while the side that hasn't been broken keeps reading its own pivot live, unaffected. The Python side (`swing_structure/premium_discount.py`) applies this to both the swing and internal tier for all three timeframes, and the FXR port now matches: all six premium/discount scripts read from `effectiveHigh`/`effectiveLow` rather than the raw pivots.
