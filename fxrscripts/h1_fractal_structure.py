// FXR Script port of the H1 FRACTAL tier (n=2, the fast tier).
//
...
//
// n=2 is carried over unchanged from the 4H trio's own fractal tier
// (itself the Pine reference's default), not separately validated on H1.
// See h1_swing_structure.py's header for the fuller caveat about H1
// calibration.
//
...
const MY_LINEWIDTH = 5;             // Also this script's deletion tag, see below.
...
let h1FractalSwingHigh = NaN;
let h1FractalSwingHighTime = null;
let h1FractalHighCrossed = false;
let h1FractalSwingLow = NaN;
let h1FractalSwingLowTime = null;
let h1FractalLowCrossed = false;

let h1FractalPrevManualInput = false;
let h1FractalLastRestartTime = null;
let h1FractalLastSeenTime = null;

// Wilder's ATR, only accumulated when the significance filter is switched
// on. Mirrors swing_structure/atr.py.
let h1FractalAtr = NaN;
let h1FractalTrSum = 0;
let h1FractalTrCount = 0;
...
init = () => {
  ...
  input.int('H1 Fractal Periods', 2, 'periods', 2, 100, 1, 'Candles required strictly beyond the pivot, on the frontier closer to now, to confirm a fractal. This is the tier\'s scale: 2 is minor pulls, 20 is major legs.', 'H1 Fractal Structure Settings');
  ...
  input.float('H1 Fractal ATR Separation', 0, 'minAtrSeparation', 0, 5, 0.25, 'Minimum leg size against the last opposite pivot, in ATRs. 0 disables the filter entirely.', 'H1 Fractal Structure Settings');
  input.int('H1 Fractal ATR Period', 14, 'atrPeriod', 2, 200, 1, 'How many True Range values feed Wilder\'s ATR. Ignored while ATR Separation is 0.', 'H1 Fractal Structure Settings');

  input.bool('H1 Fractal Manual Restart Now', false, 'manualRestartInput');
};
...
const h1FractalNearFrontierStrict = (periods, pivotValue, higher, getValue) => {
...
const h1FractalFarFrontierTolerant = (periods, pivotValue, higher, getValue, tieTolerance) => {
...
onTick = (length, _moment, _, ta, inputs) => {
  ...
  if (inferredSpacing === null) {
    ...
    // touch h1FractalLastSpacing, so a warming-up candle never looks like
    // a timeframe change.
    return;
  }
  ...
  if (inferredSpacing !== MY_TIMEFRAME_MS) {
    ...
    h1FractalLastSpacing = inferredSpacing;
    return;
  }

  if (inferredSpacing !== h1FractalLastSpacing) {
    ...
    h1FractalSwingHigh = NaN;
    h1FractalSwingHighTime = null;
    h1FractalHighCrossed = false;
    h1FractalSwingLow = NaN;
    h1FractalSwingLowTime = null;
    h1FractalLowCrossed = false;
    h1FractalPrevManualInput = false;
    h1FractalLastRestartTime = null;
    h1FractalLastSeenTime = null;
    h1FractalAtr = NaN;
    h1FractalTrSum = 0;
    h1FractalTrCount = 0;
    h1FractalStructure = null;
    h1FractalLastSpacing = inferredSpacing;
  }
  ...
  if (currentTime === h1FractalLastSeenTime) {
    return;
  }
  h1FractalLastSeenTime = currentTime;
  ...
  if (filterOn) {
    ...
    if (Number.isNaN(h1FractalAtr)) {
      h1FractalTrSum += trToday;
      h1FractalTrCount += 1;
      if (h1FractalTrCount === atrPeriod) {
        h1FractalAtr = h1FractalTrSum / atrPeriod;
      }
    } else {
      h1FractalAtr = (h1FractalAtr * (atrPeriod - 1) + trToday) / atrPeriod;
    }
  }

  const manualRaw = manualRestartInput;
  const manualTriggered = manualRaw && !h1FractalPrevManualInput;
  ...
  if (manualTriggered) {
    ...
    // become confirmable.
    h1FractalSwingHigh = high(0);
    h1FractalSwingHighTime = currentTime;
    h1FractalHighCrossed = false;
    h1FractalSwingLow = low(0);
    h1FractalSwingLowTime = currentTime;
    h1FractalLowCrossed = false;
    h1FractalLastRestartTime = currentTime;
  } else {
    ...
    if (!Number.isNaN(h1FractalSwingHigh) && !h1FractalHighCrossed && closeToday > h1FractalSwingHigh) {
      trendLine(
        newPoint(h1FractalSwingHighTime, h1FractalSwingHigh),
        newPoint(currentTime, h1FractalSwingHigh),
        { linecolor: color.red, linewidth: MY_LINEWIDTH, linestyle: 1 }
      );
      h1FractalHighCrossed = true;
      highBrokeReal = true;
    }
    if (!Number.isNaN(h1FractalSwingLow) && !h1FractalLowCrossed && closeToday < h1FractalSwingLow) {
      trendLine(
        newPoint(h1FractalSwingLowTime, h1FractalSwingLow),
        newPoint(currentTime, h1FractalSwingLow),
        { linecolor: color.green, linewidth: MY_LINEWIDTH, linestyle: 1 }
      );
      h1FractalLowCrossed = true;
      lowBrokeReal = true;
    }
    ...
    const pivotAllowed = h1FractalLastRestartTime === null || pivotTime >= h1FractalLastRestartTime;
    ...
    if (pivotAllowed && !Number.isNaN(pivotTime)) {
      ...
      const atrKnown = filterOn && !Number.isNaN(h1FractalAtr);

      const pivotHigh = high(periods);
      const isUpFractal =
        h1FractalNearFrontierStrict(periods, pivotHigh, true, (k) => high(k)) &&
        h1FractalFarFrontierTolerant(periods, pivotHigh, true, (k) => high(k), FRACTAL_TIE_TOLERANCE);
      if (isUpFractal) {
        // Leg measured down to the last confirmed LOW: asks "was the swing
        // big enough", not "did the high move far enough".
        let keep = true;
        if (atrKnown && !Number.isNaN(h1FractalSwingLow)) {
          keep = Math.abs(pivotHigh - h1FractalSwingLow) >= minAtrSeparation * h1FractalAtr;
        }
        if (keep) {
          h1FractalSwingHigh = pivotHigh;
          h1FractalSwingHighTime = pivotTime;
          h1FractalHighCrossed = false;
        }
      }
      ...
      const isDownFractal =
        h1FractalNearFrontierStrict(periods, pivotLow, false, (k) => low(k)) &&
        h1FractalFarFrontierTolerant(periods, pivotLow, false, (k) => low(k), FRACTAL_TIE_TOLERANCE);
      if (isDownFractal) {
        let keep = true;
        if (atrKnown && !Number.isNaN(h1FractalSwingHigh)) {
          keep = Math.abs(pivotLow - h1FractalSwingHigh) >= minAtrSeparation * h1FractalAtr;
        }
        if (keep) {
          h1FractalSwingLow = pivotLow;
          h1FractalSwingLowTime = pivotTime;
          h1FractalLowCrossed = false;
        }
      }
    }
  ...
  // A close cannot be both above h1FractalSwingHigh and below
  // h1FractalSwingLow at once, so these are mutually exclusive.
  if (highBrokeReal) {
    h1FractalStructure = 'bullish';
  } else if (lowBrokeReal) {
    h1FractalStructure = 'bearish';
  }

  h1FractalPrevManualInput = manualRaw;
};
