// FXR Script visualization of the Daily SWING tier's equilibrium price
// (n=20). Not an independent detector: this is daily_swing_structure.py's
...
// disagrees with daily_swing_structure.py for the same candles, that is a
// copy-drift bug, fix daily_swing_structure.py's copy first if the two were
...
// from whatever dailySwingSwingHigh/dailySwingSwingLow/the running extremes are
...
// (daily_internal_premium_discount.py) each use a different EQ_LINE_COLOR,
...
const MY_LINEWIDTH = 9;             // Same tag daily_swing_structure.py uses for its own trendLines.
...
// dailySwingSwingHigh/dailySwingSwingLow directly. Mirrors
// swing_structure/current_range.py's compute_current_range: dailySwingSwingHigh
...
let dailySwingRunningMaxHigh = NaN;
let dailySwingPrevSwingHighForRange = NaN;
let dailySwingRunningMinLow = NaN;
let dailySwingPrevSwingLowForRange = NaN;
...
init = () => {
  ...
  input.int('Daily Swing Periods', 20, 'periods', 2, 100, 1, 'Candles required strictly beyond the pivot, on the frontier closer to now, to confirm a fractal. This is the tier\'s scale: 2 is minor pulls, 20 is major legs.', 'Daily Swing Premium/Discount Settings');
  ...
  input.float('Daily Swing ATR Separation', 0, 'minAtrSeparation', 0, 5, 0.25, 'Minimum leg size against the last opposite pivot, in ATRs. 0 disables the filter entirely.', 'Daily Swing Premium/Discount Settings');
  input.int('Daily Swing ATR Period', 14, 'atrPeriod', 2, 200, 1, 'How many True Range values feed Wilder\'s ATR. Ignored while ATR Separation is 0.', 'Daily Swing Premium/Discount Settings');
...
onTick = (length, _moment, _, ta, inputs) => {
  ...
  if (inferredSpacing !== dailySwingLastSpacing) {
    ...
    dailySwingRunningMaxHigh = NaN;
    dailySwingPrevSwingHighForRange = NaN;
    dailySwingRunningMinLow = NaN;
    dailySwingPrevSwingLowForRange = NaN;
    dailySwingLastSpacing = inferredSpacing;
  }
  ...
  // this fix. ----
  if (!Number.isNaN(dailySwingSwingHigh)) {
    if (Number.isNaN(dailySwingPrevSwingHighForRange) || dailySwingSwingHigh !== dailySwingPrevSwingHighForRange) {
      dailySwingRunningMaxHigh = high(0);
    } else {
      dailySwingRunningMaxHigh = Math.max(dailySwingRunningMaxHigh, high(0));
    }
  }
  if (!Number.isNaN(dailySwingSwingLow)) {
    if (Number.isNaN(dailySwingPrevSwingLowForRange) || dailySwingSwingLow !== dailySwingPrevSwingLowForRange) {
      dailySwingRunningMinLow = low(0);
    } else {
      dailySwingRunningMinLow = Math.min(dailySwingRunningMinLow, low(0));
    }
  }
  dailySwingPrevSwingHighForRange = dailySwingSwingHigh;
  dailySwingPrevSwingLowForRange = dailySwingSwingLow;

  const effectiveHigh = Number.isNaN(dailySwingSwingHigh) ? NaN : Math.max(dailySwingSwingHigh, dailySwingRunningMaxHigh);
  const effectiveLow = Number.isNaN(dailySwingSwingLow) ? NaN : Math.min(dailySwingSwingLow, dailySwingRunningMinLow);

  // ---- Equilibrium of the CURRENT range only, appended to
  // daily_swing_structure.py's engine. equilibrium = (effectiveHigh +
  ...
  const haveEquilibrium = !Number.isNaN(equilibrium) && dailySwingStructure !== null;
